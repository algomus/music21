...
#(map
  ...
  `(
    (no-arrowhead ,boolean?
      "If set, do not draw an arrowhead on a frame extender line.")
    (extender-Y-offset ,number?
      "Vertical displacement of extender line from center staff line")
    (no-bracket ,boolean?
      "If set, do not draw bracket in frame notation.")
  ))
...
"Draw a box around a group of notes for use in frame notation."
  (let* ((staff-space (ly:staff-symbol-staff-space grob)) ; for scaling
	 ...
	 ; recompute X-ext and Y-ext (todo: on stencil ?)
	 ...
	 (let (set! height (cons
         ...
         (stencil (ly:make-stencil
		   (ly:stencil-expr (ly:round-filled-box width height 2))
		   X-ext Y-ext))
...
"Draw a bracket for use in frame notation."
  (let* ((orig (ly:grob-original grob))
         ...
         (text (ly:stencil-aligned-to text X LEFT))
         ;; (text (ly:stencil-translate-axis text (car width) X))
         (text (ly:stencil-translate-axis text (/ (interval-length width) 2) X))
         (stil point-stencil) ; (make-line-stencil line-th
                              ;    (car width) 0
                              ;    (cdr width) 0))
         (dir (ly:grob-property grob 'direction))
         ;; create the vertical lines for bracket edges
         (edge-height 0) ; (* 1.0 staff-space))
         (protrusion point-stencil) ; (make-line-stencil line-th
                                    ; 0 0
                                    ; 0 (* edge-height (- dir))))
   )
...
boxStart = \frameStart
boxEnd = { \frameEnd \frameExtenderEnd }
